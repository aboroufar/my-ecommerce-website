...
export default async function PromoPage({
  ...
  // full category list (unlike /products, which narrows to the active
  // category's own subtree) so a shopper can jump straight to any sale
  // category without leaving the promo listing.
    ...
    <main className="w-full flex-1">
      <div className="border-b border-line bg-surface px-6 py-4">
        <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-4">
          ...
          <Breadcrumb categoryName={activeCategory?.name} t={t} />
        </div>
      </div>
        ...
        <div className="min-w-0 flex-1">
          <div className="mb-10">
            ...
            <h1 className="mt-2 font-display text-3xl font-bold text-foreground">
              {activeCategory ? t("onSaleCategory", { category: activeCategory.name }) : t("onSaleNow")}
            </h1>
          </div>

          {saleProducts.length === 0 ? (
            <EmptyState hasFilter={!!category} t={t} />
          ) : (
...
function Breadcrumb({
  ...
  return (
    ...
    >
      ...
      <Link href="/promo" className="transition-colors hover:text-foreground">
        {t("promo")}
      </Link>
      {categoryName && (
        <>
