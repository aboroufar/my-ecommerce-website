...
import { createOrderTagInline, setOrderTags } from "@/lib/actions/orderNotesAndTags";

export interface OrderTagOption {
  id: string;
  name: string;
}

/**
 * Self-contained tag editor for the order detail page's sidebar --
 * direct copy of ClientTagChecklist retargeted at orders. Saves
 * immediately via setOrderTags on every toggle/add since the order
 * detail page has no single page-level submit to report back to.
 */
export function OrderTagChecklist({
  orderId,
  tags,
  selectedIds,
}: {
  ...
  function toggle(id: string) {
    ...
    startTransition(async () => {
      await setOrderTags(orderId, Array.from(next));
    });
  }
  ...
  function handleAddTag() {
    ...
    setError(null);
    startTransition(async () => {
      const result = await createOrderTagInline(name);
      if ("error" in result) {
